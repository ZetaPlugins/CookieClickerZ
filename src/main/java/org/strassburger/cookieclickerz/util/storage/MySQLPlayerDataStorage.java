...
public class MySQLPlayerDataStorage implements PlayerDataStorage {
    ...
    public void init() {
        try (Connection connection = createConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                CookieClickerZ.getInstance().getLogger().severe("Failed to initialize MySQL database: " + e.getMessage());
            }
        } catch (SQLException e) {
            CookieClickerZ.getInstance().getLogger().severe("Failed to initialize MySQL database: " + e.getMessage());
        }
    ...
    private Connection createConnection() {
        ...
        try {
            ...
            CookieClickerZ.getInstance().getLogger().severe("Failed to create connection to MySQL database: " + e.getMessage());
            return null;
        }
    }
    ...
    public void save(PlayerData playerData) {
        try (Connection connection = createConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                CookieClickerZ.getInstance().getLogger().severe("Failed to save player data to MySQL database: " + e.getMessage());
            }
        } catch (SQLException e) {
            CookieClickerZ.getInstance().getLogger().severe("Failed to save player data to MySQL database: " + e.getMessage());
        }
        ...
        try (Connection connection = createConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM hearts WHERE uuid = ?")) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    ...
                    CookieClickerZ.getInstance().getLogger().severe("Failed to load player data from MySQL database: " + e.getMessage());
                    return null;
                }
            } catch (SQLException e) {
                CookieClickerZ.getInstance().getLogger().severe("Failed to load player data from MySQL database: " + e.getMessage());
